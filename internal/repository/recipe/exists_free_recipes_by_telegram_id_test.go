...
func TestExistsFreeRecipesByTelegramID(t *testing.T) {
	type in struct {
		ctx        context.Context
		telegramID string
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[check exists free recipes by telegram id] execute repository")
			},
			in: in{
				ctx:        ctx,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[check exists free recipes by telegram id] execute repository")
				m.On("Error", "request timed out while check exists free recipes by telegram id", "err", context.DeadlineExceeded)
			},
			in: in{
				ctx:        ctx,
				telegramID: telegramID,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[check exists free recipes by telegram id] execute repository")
				m.On("Error", "failed to check exists free recipes by telegram id", "err", errors.New("database error"))
			},
			in: in{
				ctx:        ctx,
				telegramID: telegramID,
			...
			want: want{
				exists: false,
				err:    errors.New("could not check exists free recipes by telegram id: database error"),
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			result, err := repository.ExistsFreeRecipesByTelegramID(test.in.ctx, test.in.telegramID)
