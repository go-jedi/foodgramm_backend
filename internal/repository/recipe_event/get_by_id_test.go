...
func TestGetByID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[get recipe event by id] execute repository")
			},
			in: in{
				ctx:      ctx,
				recipeID: recipeID,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[get recipe event by id] execute repository")
				m.On("Error", "request timed out while get recipe event by id", "err", context.DeadlineExceeded)
			},
			in: in{
				ctx:      ctx,
				recipeID: recipeID,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[get recipe event by id] execute repository")
				m.On("Error", "failed to get recipe event by id", "err", errors.New("database error"))
			},
			in: in{
				ctx:      ctx,
				recipeID: recipeID,
			...
			want: want{
				recipeEvent: recipeevent.Recipe{},
				err:         errors.New("could not get recipe event by id: database error"),
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			result, err := repository.GetByID(test.in.ctx, test.in.recipeID)
