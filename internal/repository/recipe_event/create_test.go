...
func TestCreate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[create a new recipe event] execute repository")
			},
			in: in{
				ctx: ctx,
				dto: dto,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[create a new recipe event] execute repository")
				m.On("Error", "request timed out while creating the recipe event", "err", context.DeadlineExceeded)
			},
			in: in{
				ctx: ctx,
				dto: dto,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[create a new recipe event] execute repository")
				m.On("Error", "failed to create the recipe event", "err", errors.New("database error"))
			},
			in: in{
				ctx: ctx,
				dto: dto,
			...
			want: want{
				recipeEvent: recipeevent.Recipe{},
				err:         errors.New("could not create recipe event: database error"),
			},
		},
	}
