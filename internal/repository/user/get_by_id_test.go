...
func TestGetByID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[get user by id] execute repository")
			},
			in: in{
				ctx:    ctx,
				userID: userID,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[get user by id] execute repository")
				m.On("Error", "request timed out while get user by id", "err", context.DeadlineExceeded)
			},
			in: in{
				ctx:    ctx,
				userID: userID,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[get user by id] execute repository")
				m.On("Error", "failed to get user by id", "err", errors.New("database error"))
			},
			in: in{
				ctx:    ctx,
				userID: userID,
			...
			want: want{
				user: user.User{},
				err:  errors.New("could not get user by id: database error"),
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			result, err := repository.GetByID(test.in.ctx, test.in.userID)
