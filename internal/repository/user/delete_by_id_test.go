...
func TestDeleteByID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "ok",
			mockPoolBehavior: func(m *poolsmocks.IPool) {
				m.On("Exec", mock.Anything, mock.Anything, id).
					Return(successCommandTag, nil)
			},
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[delete user by id] execute repository", mock.Anything)
			},
			in: in{
				ctx: ctx,
				id:  id,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[delete user by id] execute repository", mock.Anything)
				m.On("Error", "request timed out while delete user by id", "err", mock.Anything)
			},
			in: in{
				ctx: ctx,
				id:  id,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[delete user by id] execute repository", mock.Anything)
				m.On("Error", "failed to delete user by id", "err", mock.Anything)
			},
			in: in{
				ctx: ctx,
				id:  id,
			...
			want: want{
				id:  0,
				err: errors.New("could not delete user by id: some db error"),
			},
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			result, err := repository.DeleteByID(test.in.ctx, test.in.id)
			...
			assert.Equal(t, test.want.id, result)
