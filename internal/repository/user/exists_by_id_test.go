...
func TestExistsByID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[check user exists by id] execute repository")
			},
			in: in{
				ctx:    ctx,
				userID: userID,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[check user exists by id] execute repository")
				m.On("Error", "request timed out while check exists user by id", "err", context.DeadlineExceeded)
			},
			in: in{
				ctx:    ctx,
				userID: userID,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[check user exists by id] execute repository")
				m.On("Error", "failed to check exists user by id", "err", errors.New("database error"))
			},
			in: in{
				ctx:    ctx,
				userID: userID,
			...
			want: want{
				exists: false,
				err:    errors.New("could not check exists user by id: database error"),
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			result, err := repository.ExistsByID(test.in.ctx, test.in.userID)
