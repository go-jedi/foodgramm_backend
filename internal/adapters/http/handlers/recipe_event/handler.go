package recipeevent

import (
	"github.com/gin-gonic/gin"
	"github.com/go-jedi/foodgrammm-backend/config"
	"github.com/go-jedi/foodgrammm-backend/internal/middleware"
	"github.com/go-jedi/foodgrammm-backend/internal/service"
	"github.com/go-jedi/foodgrammm-backend/pkg/logger"
	"github.com/go-jedi/foodgrammm-backend/pkg/validator"
)

type Handler struct {
	recipeEventService service.RecipeEventService
	cookie             config.CookieConfig
	middleware         *middleware.Middleware
	logger             *logger.Logger
	validator          *validator.Validator
}

func NewHandler(
	recipeEventService service.RecipeEventService,
	cookie config.CookieConfig,
	middleware *middleware.Middleware,
	engine *gin.Engine,
	logger *logger.Logger,
	validator *validator.Validator,
) *Handler {
	h := &Handler{
		recipeEventService: recipeEventService,
		cookie:             cookie,
		middleware:         middleware,
		logger:             logger,
		validator:          validator,
	}

	h.initRoutes(engine)

	return h
}

func (h *Handler) initRoutes(engine *gin.Engine) {
	api := engine.Group("/v1/recipe_event", h.middleware.Auth.AuthMiddleware)
	{
		api.POST("", h.create)
		api.GET("/all", h.all)
		api.GET("/all/type/:typeID", h.allByTypeID)
		api.GET("/id/:recipeID", h.getByID)
		api.PATCH("", h.update)
		api.DELETE("/id/:recipeID", h.deleteByID)
	}
}
