...
func (h *Handler) initRoutes(engine *gin.Engine) {
	api := engine.Group("/v1/admin", h.middleware.Auth.AuthMiddleware)
	{
		api.GET("/add/:telegramID", h.middleware.AdminGuard.AdminGuardMiddleware, h.addAdminUser)
		api.GET("/exists/:telegramID", h.existsByTelegramID)
	}
}
